# Copyright (c) 2026 Pyry Haulos
#
# This software is released under the MIT License.
# https://opensource.org/licenses/MIT

"""Thread-to-conversation mapping persistence for Slack.

Maps Slack threads (channel_id, thread_ts) to Airut conversation IDs.
Uses a JSON file in the repo's state directory for persistence.
"""

from __future__ import annotations

import json
import logging
import threading
from pathlib import Path


logger = logging.getLogger(__name__)


class SlackThreadStore:
    """File-backed thread-to-conversation mapping.

    Thread-safe. The mapping is loaded into memory at construction and
    written back to disk on every mutation.

    The key format is ``{channel_id}:{thread_ts}``.
    """

    def __init__(self, state_dir: Path) -> None:
        """Initialize the thread store.

        Args:
            state_dir: Directory for persistent state files.
        """
        self._path = state_dir / "slack_threads.json"
        self._lock = threading.Lock()
        self._data: dict[str, str] = {}  # "channel_id:thread_ts" -> conv_id
        self._load()

    def _load(self) -> None:
        """Load existing mappings from disk."""
        if not self._path.exists():
            return
        try:
            with open(self._path) as f:
                self._data = json.load(f)
            logger.debug(
                "Loaded %d thread mappings from %s",
                len(self._data),
                self._path,
            )
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(
                "Failed to load thread store from %s: %s", self._path, e
            )
            self._data = {}

    def _save(self) -> None:
        """Persist mappings to disk."""
        self._path.parent.mkdir(parents=True, exist_ok=True)
        try:
            with open(self._path, "w") as f:
                json.dump(self._data, f, indent=2)
        except OSError as e:
            logger.error("Failed to save thread store to %s: %s", self._path, e)

    @staticmethod
    def _key(channel_id: str, thread_ts: str) -> str:
        """Build the mapping key."""
        return f"{channel_id}:{thread_ts}"

    def get_conversation_id(
        self, channel_id: str, thread_ts: str
    ) -> str | None:
        """Look up Airut conversation ID for a Slack thread.

        Args:
            channel_id: Slack DM channel ID.
            thread_ts: Thread timestamp.

        Returns:
            Conversation ID if found, None otherwise.
        """
        with self._lock:
            return self._data.get(self._key(channel_id, thread_ts))

    def register(self, channel_id: str, thread_ts: str, conv_id: str) -> None:
        """Register a new thread-to-conversation mapping and persist.

        Args:
            channel_id: Slack DM channel ID.
            thread_ts: Thread timestamp.
            conv_id: Airut conversation ID.
        """
        with self._lock:
            self._data[self._key(channel_id, thread_ts)] = conv_id
            self._save()
            logger.debug(
                "Registered thread %s:%s -> %s",
                channel_id,
                thread_ts,
                conv_id,
            )
