...
def _is_service_installed() -> bool:
    ...
    from airut.install_services import get_systemd_user_dir
...
def cmd_check(argv: list[str]) -> int:
    ...
    # ── Version ─────────────────────────────────────────────────
    from airut.version import check_upstream_version, get_git_version_info
...
def cmd_run_gateway(argv: list[str]) -> int:
    ...
    """
    from airut.gateway.service import main as gateway_main
...
def cmd_install_service(argv: list[str]) -> int:
    ...
    Returns:
        Exit code (0 on success, 1 on error).
    """
    from airut.install_services import install_services
...
_SUBCOMMAND_HELP: dict[str, str] = {
    ...
    "run-gateway": (
        "usage: airut run-gateway [--config PATH] [--resilient] [--debug]\n\n"
