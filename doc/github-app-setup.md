...
No repository-side configuration is needed. The `GH_TOKEN` env var is
auto-injected into the container from the `github_app_credentials` key name. The
container receives a surrogate token; the proxy transparently replaces it with a
short-lived installation token for requests to scoped hosts. See
