...
from lib.gateway.config import get_config_path
...
def migrate_config(repo_root: Path) -> None:
    """Migrate config file from installation directory to XDG location.

    If ``~/.config/airut/airut.yaml`` does not exist but
    ``<repo_root>/config/airut.yaml`` does, moves the file to the XDG
    location.  This provides transparent upgrades for existing
    installations.
    ...
    logger.info("Migrating config file: %s -> %s", legacy_config, xdg_config)
