...
Accepts form-encoded parameters (htmx sends `hx-vals` as form data):
`type=list_item|dict_entry|collection_entry|tagged_union_item&path=repos.my-project.email.authorized_senders&index=0`.
For `tagged_union_item`, also accepts `rules` as a JSON string.
...
| File                                                          | Purpose                                                              |
| ------------------------------------------------------------- | -------------------------------------------------------------------- |
| `airut/config/editor.py`                                      | `EditorFieldSchema`, `schema_for_editor()`, `build_editor_context()` |
| `airut/config/editor_form.py`                                 | `form_to_raw_dict()`, raw value helpers, `InMemoryConfigSource`      |
...
| `airut/dashboard/templates/components/config/scalar.html`     | Scalar input + source selector (includes bool via `<select>`)        |
