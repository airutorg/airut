...
docker/
  ...
  proxy-allowlist.py              # proxy allowlist addon
```
...
- Generate a minimal Dockerfile in memory:
  ...
  ```
- Build with the entrypoint copied into a temporary build context
...
| Entrypoint changed                              | Rebuild overlay only (repo image reused) |
