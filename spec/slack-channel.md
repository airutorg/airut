...
repo are supported — see [multi-channel.md](multi-channel.md).
...
1. **Acknowledgment**: When a message is received, the adapter calls
   `assistant.threads.setStatus("is working on this...")` to show a loading
   indicator and posts a message to the thread: "Your request has been received
   and is now being processed by {model}." If a dashboard URL is configured, the
...
Slack's `files.getUploadURLExternal` + `files.completeUploadExternal` flow.
...
Grants access to all full workspace members. At runtime, the adapter calls
`users.info` on the sender and checks:
...
[multi-channel.md](multi-channel.md)). The `slack:` block lives under
`repos.<name>`:
...
be present per repo. See [multi-channel.md](multi-channel.md) for the full
...
This is critical. The earlier implementation attempt (PR #147) bypassed these
protocols, leading to `_is_slack` boolean branching in `RepoHandler`, type
checker exclusions, and a keep-alive thread hack. The current architecture makes
this unnecessary — `RepoHandler` remains fully channel-agnostic, calling
`adapter.listener.start()` / `adapter.listener.stop()` with no channel-specific
code paths.
...
class SlackChannelListener:
    ...
    def __init__(self, config: SlackChannelConfig) -> None:
        ...
        # Register Agents & AI Apps event handlers via Assistant middleware

    def start(self, submit: Callable[[RawMessage[Any]], bool]) -> None:
        """Connect Socket Mode and start receiving events.

        Registers the submit callback, then calls handler.connect()
...
| `send_acknowledgment()`    | Register thread mapping, `setStatus(...)` + `chat.postMessage` with confirmation text and optional dashboard link                               |
...
The existing `adapter_factory.py` dispatches on channel config type. Adding
Slack is a single `elif` branch:
...
def create_adapter(config: RepoServerConfig) -> ChannelAdapter:
    ...
    # Deferred import: slack-bolt is an optional dependency
    from airut.gateway.slack.config import SlackChannelConfig

    if isinstance(config.channel, SlackChannelConfig):
...
With multi-channel support ([multi-channel.md](multi-channel.md)), this becomes
`create_adapters()` returning a dict, but the dispatch logic per channel config
is the same.
...
a separate concern handled in [multi-channel.md](multi-channel.md).
...
With multi-channel ([multi-channel.md](multi-channel.md)), the dashboard's
`RepoState` gains per-channel health info via `ChannelInfo` objects. This is
independent of the Slack implementation itself.
...
Features to implement after the core Slack channel is working. These build on
the initial implementation and add significant value.

### Task Progress via Plan Blocks

Show real-time task progress in Slack using Slack's streaming API and plan/task
blocks. The dashboard already has live TodoWrite progress — this brings the same
visibility directly into the Slack thread.

**How it works**: When Claude calls `TodoWrite` during execution, the gateway
already captures these events for the dashboard's live progress display. For
Slack, the adapter uses the streaming API (`chat.startStream` /
`chat.appendStream` / `chat.stopStream`) to push task updates to the thread as
they happen:

- **`TaskUpdateChunk`** — represents an individual task step with `id`, `title`,
  `status` (`"pending"`, `"in_progress"`, `"complete"`, `"error"`), `details`,
  and `output` fields.
- **`PlanUpdateChunk`** — groups tasks under a named plan with a `title`.
- **`MarkdownTextChunk`** — standard markdown text interspersed with task
  updates.

The `task_display_mode` parameter on `chat.startStream` controls how tasks
appear: as a timeline of individual cards or grouped into a plan view. The plan
view is more appropriate for Airut since Claude's TodoWrite actions map directly
to a plan with multiple tasks.

```
User sends message
  -> Acknowledgment posted (existing)
  -> chat.startStream opens a streaming message
  -> As Claude calls TodoWrite:
     -> chat.appendStream with TaskUpdateChunk for each todo item
     -> Status transitions: pending -> in_progress -> complete
  -> When Claude finishes:
     -> chat.stopStream with final markdown blocks containing the response
```

This replaces the simple "is working on this..." status indicator with a
detailed progress view. The status indicator remains as a fallback for tasks
where Claude doesn't use TodoWrite.

**Implementation**: This feature requires subscribing to TodoWrite events from
the sandbox execution pipeline. The dashboard's SSE infrastructure already does
this — the Slack adapter taps into the same event stream. Bolt for Python
provides a `chat_stream()` utility that manages the start/append/stop lifecycle.

**Rate limits**: `chat.appendStream` should not be called more than once per
second to avoid rate limits. Buffer TodoWrite events and flush periodically.

### Suggested Prompts

Show configurable suggested prompts when a user opens the Chat tab (e.g.,
"Review the latest PR", "Fix failing CI"). Defined in repo config under
`slack.suggested_prompts`:

```yaml
slack:
  suggested_prompts:
    - title: Review latest PR
      message: Review the most recent open PR and leave feedback
    - title: Fix failing CI
      message: Check the CI status and fix any failing tests
```

Uses `set_suggested_prompts()` in the `thread_started` handler.

## Future Extensions

These are not part of the initial scope but could be added later without
architectural changes:

- **Model selection**: Email uses subaddressing (`airut+opus@`). For Slack, the
  user could specify a model via a command prefix or suggested prompts.
  Initially the default model from repo config is used for all Slack
  conversations.
- **Channel @mentions**: Support `@mention` in channels as a lightweight
  interaction path for one-shot requests. Would use the `app_mention` event and
  `app_mentions:read` + `channels:history` scopes. Replies would go in the
  channel thread. Could include a note like "For a longer conversation, open me
  from the sidebar."
- **Response streaming**: Use the streaming API to show Claude's final response
  as it generates, rather than waiting for the complete response. Adds
  complexity (rate limiting, buffer management, fallback handling) for marginal
  UX benefit given the async execution model. The plan blocks feature above
  covers the more valuable progress visibility use case.
- **Multi-repo routing**: A single Slack app serving multiple repos with
  thread-level routing.
- **Canvas integration**: Using Slack Canvases for long-form output that exceeds
  message size limits.
- **Slack Marketplace distribution**: The app is installed directly to a
  workspace for now, not distributed via Marketplace.
