...
The `channel` field (singular `EmailChannelConfig`) becomes `channels` (mapping
of channel type to config):
...
The factory function returns a dict instead of a single adapter:
...
`_process_message_worker` currently resolves the adapter from
`repo_handler.adapter` (line 631). With multi-channel, it receives the adapter
...
class ChannelInfo:
    ...
    info: str  # "mail.example.com", "workspace-name"
...
```json
{
  ...
  "channels": [
    ...
    {"type": "slack", "info": "my-workspace", "health": "connected"}
  ]
}
```
...
03. **`create_adapter` → `create_adapters`** — Return
...
09. **Update `multi-repo.md` and `repo-config.md`** — Reflect the multi-channel
    model in existing specs.
...
Depends on the Slack channel spec (PR #136). Adds `SlackChannelConfig`,
`SlackChannelAdapter`, `SlackChannelListener`, and `SlackThreadStore` alongside
