...
2. **Reconstruct the canonical request** from the wire request:
   ...
   - **CanonicalURI**: URI-encode each path segment, preserving `/` separators.
     Empty path becomes `/`.
   ...
   - **SignedHeaders**: Semicolon-separated lowercase header names (from the
     parsed Authorization header — reuse as-is).
...
- Normalize the path: remove redundant `/` sequences, resolve `.` and `..`
- **Exception for S3**: S3 does **not** normalize paths (double slashes and `.`
  / `..` are preserved). The proxy should detect S3 requests (service=`s3` in
  the credential scope) and skip path normalization.
...
The proxy reads the `x-amz-content-sha256` header and uses it as-is in the
canonical request. The proxy does **not** hash the body itself — it trusts the
SDK's declared payload hash. This is safe because:

1. AWS verifies the payload hash server-side
2. For `UNSIGNED-PAYLOAD`, there is nothing to verify
3. For streaming uploads, the hash indicates the streaming mode
...
The re-signing logic runs in `docker/proxy_filter.py` inside the proxy
...
The proxy Dockerfile (`docker/proxy.dockerfile`) needs `cryptography` added to
its pip install.
