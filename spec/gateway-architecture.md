...
| **Conversation timeout** | Garbage collect conversations with no activity for 7 days (configurable via `execution.conversation_max_age_days`). After deletion, `ChannelAdapter.cleanup_conversations()` is called with the surviving conversation IDs so adapters can prune stale per-conversation state (e.g. Slack thread mappings). The housekeeping thread runs 60 seconds after boot and every 24 hours thereafter, and also prunes dangling/old container images when `execution.image_prune` is `true` (default). |
