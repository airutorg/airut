...
def test_connect_success(email_config):
    ...
    with patch("imaplib.IMAP4_SSL") as mock_imap:
        ...
        mock_imap.assert_called_once_with(
            email_config.channels["email"].imap.server,
            email_config.channels["email"].imap.port,
            timeout=10,
        )
        mock_conn.login.assert_called_once_with(
            email_config.channels["email"].account.username,
            email_config.channels["email"].account.password,
        )
