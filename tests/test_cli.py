...
"""Tests for airut/cli.py — multi-command CLI."""
...
from airut.version import UpstreamVersion
...
class TestCheckDependency:
    ...
    @patch("airut.cli.shutil.which", return_value="/usr/bin/git")
    ...
    @patch("airut.cli.subprocess.run")
    @patch("airut.cli.shutil.which", return_value="/usr/bin/podman")
    ...
    @patch("airut.cli.shutil.which", return_value=None)
    ...
    @patch("airut.cli.subprocess.run")
    @patch("airut.cli.shutil.which", return_value="/usr/bin/gh")
    ...
    @patch("airut.cli.subprocess.run", side_effect=FileNotFoundError)
    @patch("airut.cli.shutil.which", return_value="/usr/bin/broken")
    ...
    @patch("airut.cli.subprocess.run")
    @patch("airut.cli.shutil.which", return_value="/usr/bin/weird")
    ...
    @patch("airut.cli.subprocess.run")
    @patch("airut.cli.shutil.which", return_value="/usr/bin/tool")
...
class TestCmdInit:
    def test_creates_config(self, tmp_path: Path) -> None:
        ...
        with patch("airut.cli.get_config_path", return_value=config_path):
            result = cmd_init([])
...
class TestUseColor:
    ...
    def test_non_tty(self) -> None:
        ...
        with patch("airut.cli.sys.stdout") as mock_stdout:
    ...
    def test_tty_with_no_overrides(self) -> None:
        ...
        with (
            ...
            patch("airut.cli.sys.stdout") as mock_stdout,
        ):
...
class TestServiceStatus:
    def test_installed_when_unit_exists(self, tmp_path: Path) -> None:
        ...
        with patch(
            "airut.install_services.get_systemd_user_dir",
...
class TestFetchRunningVersion:
    @patch("airut.cli.urllib.request.urlopen")
    ...
    @patch(
        "airut.cli.urllib.request.urlopen",
...
class TestHasVersionMismatch:
    ...
    def test_mismatch_detected(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch(
                "airut.cli.ServerConfig.from_yaml",
...
class TestGetActiveTaskCounts:
    ...
    @patch("airut.cli.ServerConfig.from_yaml", side_effect=ValueError("bad"))
    @patch("airut.cli.get_config_path")
    def test_bad_config(
        self,
        mock_config_path: MagicMock,
...
def _check_patches(
    ...
    *,
    ...
    stack.enter_context(
        patch(
            "airut.cli._is_service_installed",
            return_value=service_installed,
        )
    )
    stack.enter_context(
        patch(
            "airut.cli._is_service_running",
...
"""
    ...
    def test_version_fetch_uses_local_url(
        ...
        stack.enter_context(
            patch("airut.cli._fetch_running_version", mock_fetch)
        )
        stack.enter_context(
            patch(
                "airut.version.check_upstream_version",
...
class TestCmdUpdate:
    @patch("airut.cli.subprocess.run")
    @patch(
        "airut.install_services.get_airut_path", return_value="/usr/bin/airut"
    )
    ...
    def test_no_service_upgrade_only(
        ...
        """Runs uv upgrade and check without service management."""
        mock_run.side_effect = [
            MagicMock(
                returncode=0,
                stdout="Updated airut v0.8.0 -> v0.9.0",
                stderr="",
            ),  # uv upgrade
    ...
    def test_service_installed_full_cycle(
        ...
        """Full cycle: upgrade, stop, check, reinstall."""
        mock_run.side_effect = [
        ...
        out = capsys.readouterr().out
        assert "Stopping service" in out
        assert "Running readiness check" in out
    ...
    @patch(
        "airut.install_services.uninstall_services",
    ...
    def test_reinstall_fails(
        ...
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # upgrade
            MagicMock(returncode=0, stdout="", stderr=""),  # check
            MagicMock(
    ...
    def test_no_output_from_upgrade(
        ...
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="", stderr=""),  # uv upgrade
            MagicMock(returncode=0, stdout="", stderr=""),  # check
        ]
        result = cmd_update([])
        assert result == 0
    ...
    @patch("airut.cli._is_service_running", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_nothing_to_upgrade_skips_service_restart(
        self,
        _color: MagicMock,
        _running: MagicMock,
        _installed: MagicMock,
        ...
        """Skips service restart when already up to date."""
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Nothing to upgrade", stderr=""
        )
        result = cmd_update([])
        assert result == 0
        # Should call uv but NOT install-service
    ...
    @patch("airut.cli._is_service_installed", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_nothing_to_upgrade_no_service(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        mock_run: MagicMock,
        ...
        """Returns 0 with 'Already up to date' when no service installed."""
        mock_run.return_value = MagicMock(
            returncode=0, stdout="Nothing to upgrade", stderr=""
        )
        result = cmd_update([])
    ...
    @patch("airut.cli._get_active_task_counts", return_value=(2, 1))
    @patch("airut.cli._is_service_running", return_value=True)
    @patch("airut.cli._is_service_installed", return_value=True)
    @patch("airut.cli._use_color", return_value=False)
    ...
    @patch("airut.cli._is_service_installed", return_value=True)
    @patch("airut.cli._use_color", return_value=False)
    def test_force_bypasses_task_check(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        _running: MagicMock,
        ...
        with patch("airut.install_services.uninstall_services"):
            with patch(
                "airut.install_services.get_airut_path",
    ...
    @patch("airut.cli.time.sleep")
    @patch("airut.cli._get_active_task_counts")
    @patch("airut.cli._is_service_running", return_value=True)
    @patch("airut.cli._is_service_installed", return_value=True)
    @patch("airut.cli._use_color", return_value=False)
    def test_wait_polls_until_idle(
        ...
        with (
            patch("airut.cli.subprocess.run") as mock_run,
            patch("airut.install_services.uninstall_services"),
            patch(
                "airut.install_services.get_airut_path",
                return_value="/usr/bin/airut",
    ...
    @patch("airut.cli.subprocess.run")
    @patch("airut.cli._get_active_task_counts", return_value=(0, 0))
    @patch("airut.cli._is_service_running", return_value=True)
    @patch("airut.cli._is_service_installed", return_value=True)
    @patch("airut.cli._use_color", return_value=False)
    ...
    @patch("airut.cli._is_service_installed", return_value=True)
    @patch("airut.cli._use_color", return_value=False)
    def test_service_not_running_skips_task_check(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        _running: MagicMock,
    ...
    @patch("airut.cli._is_service_running", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_readiness_check_failure_stops_service(
        self,
        _color: MagicMock,
        _running: MagicMock,
        _installed: MagicMock,
        ...
        """Returns 1 with service stopped when readiness check fails."""
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # uv upgrade
            MagicMock(
                returncode=1,
        ...
        result = cmd_update([])
        assert result == 1
        # Service was stopped before the readiness check
        mock_uninstall.assert_called_once()
        # Only 2 subprocess calls: upgrade + check (no install-service)
        assert mock_run.call_count == 2
        out = capsys.readouterr().out
        assert "Stopping service" in out
        assert "Readiness check failed" in out
        assert "missing field" in out
        assert "Service is stopped" in out
        assert "airut install-service" in out
        assert "Reinstalling" not in out
    ...
    @patch("airut.cli._is_service_installed", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_readiness_check_failure_no_service(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        _airut_path: MagicMock,
        ...
        """Returns 1 when readiness check fails even without service."""
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # uv upgrade
            MagicMock(
                returncode=1,
        ...
        assert "airut install-service" in out
        assert "Service is stopped" not in out
        assert "Stopping service" not in out
    ...
    @patch("airut.cli._is_service_installed", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_readiness_check_timeout(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        _airut_path: MagicMock,
        ...
        """Returns 1 when readiness check subprocess times out."""
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # uv upgrade
            subprocess.TimeoutExpired(cmd="airut", timeout=30),  # check
        ]
        result = cmd_update([])
        assert result == 1
        out = capsys.readouterr().out
    ...
    @patch("airut.cli._is_service_running", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_readiness_check_timeout_with_service(
        self,
        _color: MagicMock,
        _running: MagicMock,
        _installed: MagicMock,
        ...
        """Stops service and returns 1 when readiness check times out."""
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # uv upgrade
            subprocess.TimeoutExpired(cmd="airut", timeout=30),  # check
        ]
    ...
    @patch("airut.cli._is_service_installed", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_readiness_check_binary_not_found(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        _airut_path: MagicMock,
        ...
        """Returns 1 when airut binary not found for readiness check."""
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # uv upgrade
            FileNotFoundError("not found"),  # check
        ]
        result = cmd_update([])
        assert result == 1
        out = capsys.readouterr().out
    ...
    @patch("airut.cli._is_service_installed", return_value=False)
    @patch("airut.cli._use_color", return_value=False)
    def test_readiness_check_failure_stderr_output(
        self,
        _color: MagicMock,
        _installed: MagicMock,
        _airut_path: MagicMock,
        ...
        """Shows stderr when check fails with stderr only."""
        mock_run.side_effect = [
            MagicMock(returncode=0, stdout="Updated", stderr=""),  # uv upgrade
            MagicMock(
                returncode=1, stdout="", stderr="Traceback: import error"
            ),  # check
        ]
        result = cmd_update([])
        assert result == 1
        out = capsys.readouterr().out
        assert "Readiness check failed" in out
        assert "Traceback: import error" in out
...
class TestCmdRunGateway:
    @patch("airut.gateway.service.main", return_value=0)
...
class TestCli:
    def test_no_args_prints_info(self) -> None:
        ...
        with (
            patch("airut.cli.sys.argv", ["airut"]),
            patch("airut.cli._print_info") as mock_info,
    ...
    def test_unknown_command_exits_with_error(
        ...
        with (
            patch("airut.cli.sys.argv", ["airut", "--resilient"]),
            pytest.raises(SystemExit) as exc_info,
        ):
            cli()
    ...
    @patch("airut.cli.sys.exit")
    @patch("airut.cli.cmd_init", return_value=0)
    def test_init_subcommand(
        self, mock_init: MagicMock, mock_exit: MagicMock
    ) -> None:
        with patch("airut.cli.sys.argv", ["airut", "init"]):
            cli()
        mock_init.assert_called_once_with([])
        mock_exit.assert_called_once_with(0)

    @patch("airut.cli.sys.exit")
    @patch("airut.cli.cmd_check", return_value=0)
    def test_check_subcommand(
        self, mock_check: MagicMock, mock_exit: MagicMock
    ) -> None:
        with patch("airut.cli.sys.argv", ["airut", "check"]):
            cli()
        mock_check.assert_called_once_with([])
        mock_exit.assert_called_once_with(0)

    @patch("airut.cli.sys.exit")
    @patch("airut.gateway.service.main", return_value=1)
    def test_nonzero_exit_propagated(
        self, mock_main: MagicMock, mock_exit: MagicMock
    ) -> None:
        with patch("airut.cli.sys.argv", ["airut", "run-gateway"]):
            cli()
        mock_exit.assert_called_once_with(1)

    @patch("airut.cli.sys.exit")
    @patch("airut.cli.cmd_update", return_value=0)
    def test_update_subcommand(
        self, mock_update: MagicMock, mock_exit: MagicMock
    ) -> None:
        with patch("airut.cli.sys.argv", ["airut", "update"]):
            cli()
        mock_update.assert_called_once_with([])
        mock_exit.assert_called_once_with(0)

    @patch("airut.cli.sys.exit")
    @patch("airut.cli.cmd_install_service", return_value=0)
    def test_install_service_subcommand(
        self, mock_install: MagicMock, mock_exit: MagicMock
    ) -> None:
        with patch("airut.cli.sys.argv", ["airut", "install-service"]):
            cli()
        mock_install.assert_called_once_with([])
        mock_exit.assert_called_once_with(0)

    @patch("airut.cli.sys.exit")
    @patch("airut.cli.cmd_uninstall_service", return_value=0)
    def test_uninstall_service_subcommand(
        self, mock_uninstall: MagicMock, mock_exit: MagicMock
    ) -> None:
        with patch("airut.cli.sys.argv", ["airut", "uninstall-service"]):
            cli()
    ...
    def test_help_flag(self) -> None:
        ...
        with (
            patch("airut.cli.sys.argv", ["airut", "--help"]),
            patch("airut.cli._print_info") as mock_info,
            pytest.raises(SystemExit) as exc_info,
        ):
            cli()
    ...
    def test_subcommand_help(self, capsys: pytest.CaptureFixture[str]) -> None:
        """``airut update --help`` prints command help, exits 0."""
        with (
            patch("airut.cli.sys.argv", ["airut", "update", "--help"]),
            pytest.raises(SystemExit) as exc_info,
        ):
            cli()
        assert exc_info.value.code == 0
        out = capsys.readouterr().out
        assert "usage: airut update" in out
        assert "--wait" in out
        assert "--force" in out

    def test_all_subcommands_have_help(self) -> None:
        """Every dispatched subcommand has an entry in _SUBCOMMAND_HELP."""
        from airut.cli import _DISPATCH

        for cmd in _DISPATCH:
            assert cmd in _SUBCOMMAND_HELP, f"Missing help for '{cmd}'"
...
class TestCmdInstallService:
    @patch("airut.install_services.install_services")
    ...
    @patch(
        "airut.install_services.install_services",
