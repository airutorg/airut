...
def canonical_uri(path: str, *, is_s3: bool = False) -> str:
    ...
    Args:
        ...
        is_s3: If True, skip path normalization (S3 preserves double
            slashes and . / ..).
    ...
    if not is_s3:
        # Normalize: resolve . and .., collapse //
...
class ResignResult:
    ...
    __slots__ = ("auth_header", "region", "is_chunked")
...
def resign_request(
    *,
    ...
    auth_header = (
        ...
        f"SignedHeaders={parsed_auth.signed_headers}, "
        f"Signature={signature}"
    )

    return ResignResult(auth_header, region, is_chunked=is_chunked)
